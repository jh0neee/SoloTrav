...
export type SafetyFacilityType = 'police' | 'store' | 'medical' | 'etc';
...
/** 응답이 배열이거나 { data: [...] } / { result: [...] } 형태여도 목록만 뽑아냅니다. */
function extractList(payload: unknown): RawFacility[] {
  if (Array.isArray(payload)) {
    return payload as RawFacility[];
  }
  if (payload && typeof payload === 'object') {
    for (const key of ['data', 'result', 'items', 'facilities', 'content']) {
...
export const sosApi = {
  ...
  safetyFacilities: async (
    ...
    try {
      ...
      return extractList(data).map(normalize).slice(0, limit);
    } catch (error) {
